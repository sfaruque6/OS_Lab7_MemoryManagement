...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
   ...
   if (!cur -> next){
     if (new -> blk -> start <= cur -> blk -> start){
       new -> next = l -> head;
       l-> head = new;
     } else{
       new -> next = NULL;
     ...
     if (new -> blk -> start <= cur -> blk -> start) {
       ...
       while (cur && new -> blk -> start > cur -> blk -> start){
         prev = cur;
         cur = cur -> next;
       }
       prev -> next = new;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
   ...
   int newsize = new -> blk -> end - new -> blk -> start;
   int cursize;
   ...
   cur = prev = l -> head;
   cursize = cur -> blk -> end - cur -> blk -> start + 1;
  if (!cur -> next){
    if (newsize <= cursize){
      new -> next = l->head;
      l->head = new;
    ...
    if (newsize <= cursize){
      ...
      while (cur && newsize > cursize) {
        prev = cur;
        cur = cur -> next;
      }
      if (cur) {
        cursize = cur ->blk->end - cur -> blk -> start;
      }
    }
    prev-> next = new;
    new -> next = cur;
