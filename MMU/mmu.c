...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  node_t* cur = freelist -> head;
  node_t* prev = cur;
  int blankSize;

  while (cur){
    blankSize = cur -> blk -> end - cur -> blk -> start;
    if (blankSize >= blocksize){
      if (prev == cur){
        if (!cur -> next){
          freelist -> head = NULL;
        }
        else{
          freelist -> head = cur->next;
        }
      } else{
        prev -> next = cur ->next;
      }
  ...
  int org;
  org = cur->blk->end;
  cur->blk->end = cur->blk->start + blocksize - 1;
  list_add_ascending_by_address(alloclist, cur->blk);
	block_t* frag = (block_t *) malloc(sizeof(block_t));
  frag -> pid = 0;
  frag -> start = cur -> blk -> end + 1;
  frag ->end = org;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
	...
	cur->next = NULL;
	if (policy == 1) {
		list_add_to_back(freelist, cur->blk);
	} else if (policy == 2) {
		list_add_ascending_by_blocksize(freelist, cur->blk);
	} else if (policy == 3) {
		list_add_descending_by_blocksize(freelist, cur->blk);
	}
}
